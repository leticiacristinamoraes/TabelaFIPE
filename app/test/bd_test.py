...
def main():
    on_startup()
    if st.button("Criar"):
        user = user_repo.create('user 1', 'email@gmail.com')
        role = role_repo.create('researcher')
        permission = permission_repo.create('edit')
        user_role = user_role_repo.create(user_id=user.id, role_id=role.id)
        role_permission = role_permission_repo.create(role_id=role.id, permission_id=permission.id)
        shop = shop_repo.create('shop 1')
        user_shop = user_shop_repo.create(user_id=user.id, shop_id=shop.id)
