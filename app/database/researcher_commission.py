...
def update_commission():
    ...
    if len(confirmacao) == None or len(confirmacao) == 0:
...
def insert_commission(mes,ano):
    ...
    if len(confirmacao) == None or len(confirmacao) == 0:
        ...
        cur.close()
        conn.close()
        return "Comissão calculada com sucesso."
    else:
        return "Comissão desta data já calculada."
...
def commission_consult(mes,ano):
    ...
    cur.execute("""
                SELECT	users.nome,
                researcher_commission.mes,
                researcher_commission.ano,
                researcher_commission.comissao
                FROM researcher_commission
                    INNER JOIN users ON users.id = researcher_commission.pesquisador_id
                        WHERE mes={} AND ano={};""".format(mes,ano))
    exibicao = cur.fetchall()
    cur.close()
    conn.close()
    df = pd.DataFrame(exibicao, columns=['Pesquisador', 'Mês', 'Ano', 'Comissão'])
    return df
